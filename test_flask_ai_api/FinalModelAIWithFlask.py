...
# Function to segment leaf (used by SVM only)
...
# Predict disease using SVM bundle (Updated with CLAHE)
def predict_disease_with_svm(raw_image, plant_type):
    ...
    # Apply CLAHE first
    clahe_image = apply_clahe(raw_image)
    
    # Preprocess image
    segmented = segment_leaf(clahe_image)
    ...
    print(f"🩺 Disease Prediction (SVM): {predicted_class}")
...
def predict():
    ...
    if results[0].masks is not None and len(results[0].masks.data) > 0:
        ...
        largest_mask = masks[np.argmax([np.sum(m) for m in masks])].astype(np.uint8) * 255
        masked_image = cv2.bitwise_and(no_bg_resized, no_bg_resized, mask=largest_mask)

        ys, xs = np.where(largest_mask > 0)
        if len(xs) == 0 or len(ys) == 0:
            return jsonify({'error': 'Empty mask detected'}), 400
        ...
        top3_dict = {label: f"{score * 100:.2f}%" for label, score in zip(top3_labels, top3_scores)}

        plant_type = top3_labels[0]
        diagnosis = predict_disease_with_svm(original_image, plant_type)
        ...
        return jsonify({'error': 'No valid mask found by YOLO'}), 400
